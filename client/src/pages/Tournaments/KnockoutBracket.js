  ...
  const generateKnockout = async (values) => {
    try {
      ...
      if (response.data.success) {
        ...
        message.info(`成功生成 ${data.selected_teams} 支隊伍的淘汰賽，共 ${data.total_rounds} 輪 ${data.total_matches} 場比賽`);
      ...
      return (
        <Card>
          <div style={{ textAlign: 'center', padding: '40px' }}>
            ...
            <Title level={4} style={{ color: '#999' }}>尚未生成淘汰賽對戰表</Title>
      ...
      <Card 
        ...
        extra={
          <Button 
            danger
            icon={<DeleteOutlined />}
            onClick={deleteAllKnockoutMatches}
            loading={deleting}
          >
            刪除所有淘汰賽
          </Button>
