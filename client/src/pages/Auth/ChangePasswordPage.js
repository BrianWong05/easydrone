      ...
      <Card className="w-full max-w-lg shadow-2xl">
        <Space direction="vertical" size="large" className="w-full">
          <div className="text-center">
            <h2 className="text-2xl font-bold mb-2 text-gray-800">{t("auth:changePassword.title")}</h2>
            <p className="text-gray-500">{t("auth:changePassword.currentUser", { username: user?.username })}</p>
          </div>

          {error && <Alert message={error} type="error" showIcon closable onClose={() => setError("")} />}

          <Form form={form} name="changePassword" onFinish={onFinish} autoComplete="off" size="large" layout="vertical">
            <Form.Item
            ...
            >
              <Input.Password prefix={<KeyOutlined />} placeholder={t("auth:changePassword.newPasswordPlaceholder")} />
            </Form.Item>
            ...
            <Form.Item>
              <Space className="w-full" direction="vertical">
                ...
                <Button type="default" onClick={() => navigate(-1)} className="w-full">
          ...
          <div className="text-center">
            <p className="text-gray-400 text-xs">{t("auth:changePassword.loginRequiredAfterChange")}</p>
          </div>
        </Space>
      </Card>
    </div>
