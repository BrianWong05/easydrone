...
import { Card, Typography, Button, Space, Tag, Descriptions, Table, Spin, message, Row, Col, Statistic, Timeline, Modal } from 'antd';
import { ArrowLeftOutlined, PlayCircleOutlined, EditOutlined, CalendarOutlined, TrophyOutlined, TeamOutlined, DeleteOutlined } from '@ant-design/icons';
...
const MatchDetail = () => {
  ...
  const getEventText = (eventType) => {
    switch (eventType) {
      case 'goal': return '進球';
      case 'foul': return '犯規';
      case 'timeout': return '暫停';
      case 'penalty': return '點球';
      case 'substitution': return '換人';
      default: return '其他';
    }
  };
      ...
      <Space direction="vertical" size="large" style={{ width: '100%' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            ...
            <Title level={2} style={{ margin: 0 }}>比賽詳情</Title>
          </div>
          ...
          <Space>
            ...
            {matchData.match_status === 'completed' && (
              <Button 
                type="primary"
                icon={<EditOutlined />}
                onClick={() => navigate(`/matches/${id}/result-edit`)}
              >
                編輯結果
              </Button>
            )}
          ...
          <Row gutter={[24, 24]}>
            <Col xs={24} lg={12}>
              <div style={{ textAlign: 'center', padding: '20px' }}>
                ...
                <div style={{ fontSize: '24px', fontWeight: 'bold', marginBottom: 16 }}>
                  <span style={{ color: '#1890ff' }}>{matchData.team1_name}</span>
                  <span style={{ margin: '0 16px', color: '#666' }}>VS</span>
                  <span style={{ color: '#1890ff' }}>{matchData.team2_name}</span>
                </div>
                {matchData.match_status !== 'pending' && (
            ...
            <Col xs={24} lg={12}>
              <Row gutter={16}>
                <Col span={12}>
                  <Statistic
                    title={`${matchData.team1_name} 犯規`}
                    value={matchData.team1_fouls}
                    prefix="🟨"
                  />
                </Col>
                <Col span={12}>
                  <Statistic
                    title={`${matchData.team2_name} 犯規`}
                    value={matchData.team2_fouls}
                    prefix="🟨"
                  />
                </Col>
              </Row>
        ...
        <Card title="比賽信息" extra={<CalendarOutlined />}>
          <Descriptions bordered column={2}>
            ...
            {matchData.overtime_time && (
              <Descriptions.Item label="延長賽時間">
                {matchData.overtime_time} 分鐘
              </Descriptions.Item>
            )}
