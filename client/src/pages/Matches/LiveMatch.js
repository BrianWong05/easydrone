...
import { Card, Typography, Button, Space, Row, Col, Statistic, message, Modal, InputNumber, Select, Input, Timeline, Tag, Spin } from 'antd';
import { ArrowLeftOutlined, PlayCircleOutlined, PauseCircleOutlined, StopOutlined, PlusOutlined, MinusOutlined, TrophyOutlined } from '@ant-design/icons';
...
const LiveMatch = () => {
    ...
    if (isRunning && matchStarted && remainingTime > 0) {
      ...
      interval = setInterval(() => {
        setRemainingTime(time => {
          ...
          const halfText = currentHalf === 1 ? '上半場' : 
                          currentHalf === 2 ? '下半場' : '延長賽';
          ...
          if (newTime <= 0) {
            ...
            if (currentHalf === 1) {
              ...
              // 下半場結束，檢查勝負條件
              const { winnerId, reason } = determineWinner(team1Score, team2Score, team1Fouls, team2Fouls, matchData.team1_id, matchData.team2_id);
  ...
  const handleStartMatch = async () => {
    try {
      const response = await axios.post(`/api/matches/${id}/start`);
      if (response.data.success) {
  ...
  const handleStartOvertime = () => {
    ...
    // 格式化顯示時間
    const displayTime = overtimeSeconds > 0 
      ? `${overtimeMinutes}分${overtimeSeconds}秒`
      : `${overtimeMinutes}分鐘`;
  ...
  const updateScore = async (scores = null) => {
    try {
      ...
      const response = await axios.put(`/api/matches/${id}/score`, scoreData);
  ...
  const handleEndMatch = async (winnerId = null, reason = null) => {
    try {
      ...
      if (winnerId === null || reason === null) {
        const result = determineWinner(team1Score, team2Score, team1Fouls, team2Fouls, matchData.team1_id, matchData.team2_id);
        winnerId = result.winnerId;
        reason = result.reason;
      }
      
      const response = await axios.post(`/api/matches/${id}/end`, {
        winner_id: winnerId,
        win_reason: reason
      });
      ...
      <Space direction="vertical" size="large" style={{ width: '100%' }}>
        ...
        <Card style={{ background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', border: 'none' }}>
          <Row gutter={[24, 24]} align="middle">
            ...
            <Col xs={24} lg={8}>
              <div style={{ textAlign: 'center' }}>
                ...
                <Text style={{ color: '#fff', fontSize: '14px' }}>
                  / {formatMatchDuration(matchData.match_time)}
                </Text>
                <div style={{ marginTop: 8 }}>
                  <Tag color={currentHalf === 1 ? 'blue' : 
                            currentHalf === 2 ? 'green' : 'red'} 
                       style={{ fontSize: '12px' }}>
                    {isHalfTime ? '中場休息' : 
                     isOvertime ? '延長賽準備' :
                     currentHalf === 1 ? '上半場' : 
            ...
            <Col xs={24} lg={8}>
              <div style={{ textAlign: 'center', color: '#fff' }}>
                <Tag color={matchData.match_status === 'active' ? 'green' : 
                           matchData.match_status === 'completed' ? 'blue' : 'orange'} 
                     style={{ fontSize: '14px', padding: '4px 12px' }}>
                  {matchData.match_status === 'pending' ? '待開始' :
                   matchData.match_status === 'active' ? '進行中' : '已完成'}
                </Tag>
              </div>
            </Col>
          </Row>
        ...
        {!matchStarted && matchData.match_status === 'pending' && (
          ...
          >
            <div style={{ textAlign: 'center', padding: '8px 0' }}>
              <Text style={{ color: '#fff', fontSize: '16px' }}>
                <kbd style={{ fontSize: '16px', padding: '6px 12px', backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: '4px' }}>Enter</kbd>
                : 開始比賽
              </Text>
            </div>
          </Card>
        ...
        {matchStarted && matchData.match_status === 'active' && (
          ...
          >
            <Row gutter={[16, 8]} style={{ color: '#fff' }}>
              <Col xs={12} sm={6}>
                <Text style={{ color: '#fff' }}><kbd>Q</kbd>/<kbd>W</kbd>: {matchData.team1_name} 得分 +/-</Text>
              </Col>
              <Col xs={12} sm={6}>
                <Text style={{ color: '#fff' }}><kbd>O</kbd>/<kbd>P</kbd>: {matchData.team2_name} 得分 +/-</Text>
              </Col>
              <Col xs={12} sm={6}>
                <Text style={{ color: '#fff' }}><kbd>A</kbd>/<kbd>S</kbd>: {matchData.team1_name} 犯規 +/-</Text>
              </Col>
              <Col xs={12} sm={6}>
                <Text style={{ color: '#fff' }}><kbd>K</kbd>/<kbd>L</kbd>: {matchData.team2_name} 犯規 +/-</Text>
              </Col>
            ...
            <Timeline>
              {events.map((event, index) => (
                <Timeline.Item 
                  ...
                  color={event.event_type === 'goal' ? 'green' : 
                         event.event_type === 'foul' ? 'red' : 'blue'}
                >
                  <div>
                    <Text strong>{event.event_time}</Text> - 
                    <Text style={{ marginLeft: 8 }}>{event.team_name}</Text>
                    <Tag color="blue" style={{ marginLeft: 8 }}>
                      {event.event_type === 'goal' ? '進球' :
                       event.event_type === 'foul' ? '犯規' : event.event_type}
                    </Tag>
                    {event.athlete_name && (
                      <Text style={{ marginLeft: 8 }}>({event.athlete_name})</Text>
                    )}
                    {event.description && (
        ...
        >
          ...
          <p>當前比分：{matchData.team1_name} {team1Score} : {team2Score} {matchData.team2_name}</p>
          <p>當前階段：{currentHalf === 1 ? '上半場' : currentHalf === 2 ? '下半場' : '延長賽'}</p>
          <p>犯規數：{team1Fouls} : {team2Fouls}</p>
          <p>當前狀態：{isHalfTime ? '中場休息' : 
                         isOvertime ? '延長賽準備' :
                         currentHalf === 1 ? '上半場' : 
                         currentHalf === 2 ? '下半場' : '延長賽'}</p>
          {(() => {
            const { winnerId, reason } = determineWinner(team1Score, team2Score, team1Fouls, team2Fouls, matchData.team1_id, matchData.team2_id);
            if (winnerId) {
              const winnerName = winnerId === matchData.team1_id ? matchData.team1_name : matchData.team2_name;
              const reasonText = getWinReasonText(reason);
              return <p style={{ color: '#52c41a', fontWeight: 'bold' }}>獲勝者：{winnerName} ({reasonText})</p>;
            } else {
        ...
        >
          ...
          <p>當前比分：{matchData.team1_name} {team1Score} : {team2Score} {matchData.team2_name}</p>
          <p>剩餘時間：{Math.floor(remainingTime / 60)}:{(remainingTime % 60).toString().padStart(2, '0')}</p>
          {currentHalf === 2 && team1Score === team2Score && (
            <p style={{ color: '#ff6b35', fontWeight: 'bold' }}>
              ⚠️ 當前比分平局，結束下半場後將進入延長賽
            </p>
          )}
        </Modal>
      </Space>
