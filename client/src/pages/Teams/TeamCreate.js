  ...
  const fetchGroups = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        console.log('📋 獲取到的小組選項:', groupsData);
      } else {
    ...
    <div style={{ padding: '24px' }}>
      <Space direction="vertical" size="large" style={{ width: '100%' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <Title level={2} style={{ margin: 0 }}>新增隊伍</Title>
        </div>
        ...
        <Card>
          ...
          >
            ...
            >
              <Select 
                placeholder={groupsLoading ? "載入小組中..." : "請選擇小組"}
                size="large"
              ...
              >
                ...
                {groups.map(group => (
                  <Option key={group.group_id} value={group.group_id}>
                    小組 {group.group_name} ({group.team_count || 0}/{group.max_teams || 4} 隊伍)
                  </Option>
                ))}
              </Select>
            </Form.Item>
