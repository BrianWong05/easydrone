              ...
              <Form.Item
                ...
                label={
                  <span className="text-sm font-medium text-gray-700">
                    {t('athlete:athlete.team')} <span className="text-gray-400 text-xs">({t('athlete:form.optional')})</span>
                  </span>
                }
              >
                <Select
              ...
              >
                ...
                >
                  {['attacker', 'defender', 'substitute'].map(position => {
                    const availability = selectedTeam ? getPositionAvailability(position) : { available: true, current: 0, max: '∞', color: 'text-gray-500' };
                    return (
                      <Option 
                        key={position} 
                        value={position}
                        disabled={selectedTeam && !availability.available}
                      >
